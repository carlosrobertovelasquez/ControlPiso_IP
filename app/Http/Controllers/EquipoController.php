...
*/
  ...
  public function ListarArticuloOperacion(Request $request){
    $id1=$_GET['art'];
    $id2=$_GET['ope'];
    $articulooperacion=DB::Connection()->select("SELECT 
    RUBRO.ARTICULO,RUBRO.OPERACION,PROCESO.DESCRIPCION, 
    RUBRO.RUBRO,LIQ.DESCRIP_RUBRO ,
    CASE  PROCESO.CP_TIEMPOCAMBIOMOLDE 
    WHEN NULL THEN PROCESO.CP_TIEMPOCAMBIOMOLDE ELSE 0.0  END as CP_TIEMPOCAMBIOMOLDE ,
    (PROCESO.CANT_PRODUCIDA_PT/PROCESO.HORAS_STD_MOE) AS HORASXHORA
          FROM 
          IBERPLAS.ESTRUC_PROC_RUBRO RUBRO,
          IBERPLAS.ESTRUC_PROCESO PROCESO,
          IBERPLAS.RUBRO_LIQ LIQ
          WHERE 
          LIQ.RUBRO=PROCESO.EQUIPO AND
          LIQ.RUBRO=RUBRO.RUBRO AND
          PROCESO.ARTICULO=RUBRO.ARTICULO AND
          PROCESO.VERSION IN(SELECT VERSION FROM IBERPLAS.ESTRUC_MANUFACTURA WHERE ESTADO='A' AND ARTICULO='$id1')AND    
          RUBRO.ARTICULO='$id1' AND 
          PROCESO.ARTICULO='$id1' AND
          RUBRO.OPERACION=PROCESO.OPERACION and
          PROCESO.DESCRIPCION='$id2' AND
